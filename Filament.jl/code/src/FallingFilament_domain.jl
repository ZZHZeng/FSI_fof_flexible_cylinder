...
u⁰ = SMatrix{2,size(mesh.controlPoints,2)}(mesh.controlPoints[1:2,:].*L.+[L,domain[2]-L])
...
global U_cm = SA[0.,0.]
global X = SA[0.,0.]
sim = CoupledSimulation(domain,(0,0),L,body,struc,IQNCoupling;
...
# store initial state
# file = jldopen("../data/fillament_domain.jld2","w")
count = 0
...
@time for tᵢ in range(t₀,t₀+duration;step)
    ...
    while WaterLily.time(sim) < tᵢ*sim.L/sim.U
        ...
        while true
            ...
            sim.body = ParametricBodies.update!(sim.body,u⁰.+sim.L*sim.pnts,sim.flow.Δt[end])
    ...
    # group["pnts"] = sim.body.curve.pnts
