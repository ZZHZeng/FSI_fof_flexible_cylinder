...
function airship(L; Re_exp=108500,U=1,T=Float32,mem=Array)#CUDA.CuArray)
...
function run(L,duration,tstep) 
    ...
    savefig(name*".png")
    plot(range(0,duration;step=tstep),-vforce,label="Cv",xlabel="tU/L",ylabel="C_force")
    savefig(name*"_vforce"*".png")
    plot(range(0,duration;step=tstep),-pforce,label="Cp",xlabel="tU/L",ylabel="C_force")
    savefig(name*"_pforce"*".png")
    plot(range(0,duration;step=tstep),ratio,label="Cv/Cp",xlabel="tU/L",ylabel="Cv/Cp")
    savefig(name*"_CvCp"*".png")
end 
