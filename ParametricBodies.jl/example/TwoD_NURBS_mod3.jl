...
function run(L,thk)
    ...
    t₀,duration,tstep = sim_time(sim),4,0.1;
    name = "2Dnurbs_test_thk$(thk)"
    ...
    for tᵢ in range(t₀,t₀+duration;step=tstep)
        ...
        while t < tᵢ*sim.L/sim.U
            # random update
            new_pnts = (SA[-1     0   1
                            0.0+sin(π/4*t/sim.L) 0.0 0.0-sin(π/4*t/sim.L)] .+ [2,2])*sim.L
        ...
        vol2 = thk*L*(2^2+(2.0*sin(π/4*tᵢ/sim.L*sim.L/sim.U))^2)^0.5 + π*(thk/2)^2 # analytical result
        push!(vol_analy,vol2)
        write!(wr, sim)
    end
    close(wr)
