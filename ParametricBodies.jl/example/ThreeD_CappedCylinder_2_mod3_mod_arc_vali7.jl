...
function pressure_force1(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    forces = []; a=[]; b=[]; c=[]; d=[];
    ds = 1/n
    dl = 1.5 # half length of the cylinder
    for i in 1:n-1
        s = i*ds 
    ...
    for i in 1:n-1
        ...
        p1 = a[i]
        p2 = b[i]
        WaterLily.@loop sim.flow.f[I,:] .= sim.flow.p[I].*(-nds1(sim.body,loc(0,I,T),p1,p2,R)) over I ∈ inside(sim.flow.p)
...
function surface_area(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for i in 1:n-1
        ...
        p1 = a[i]
        p2 = b[i]
        WaterLily.@loop sim.flow.f[I,:] .= 1*norm(nds1(sim.body,loc(0,I,T),p1,p2,R)) over I ∈ inside(sim.flow.p)
...
using ForwardDiff
function nds1(body,x,p1,p2,R) # return nds
    aa = x - p1 # vector x_p1
    bb = x - p2 # vector x_p2
...
function nds2(body,x,p1,p2,R) # return scalar
...
function run(L,thk,N)
    ...
    p1,p2,p3,p4 = a[1],b[1],a[1],b[1]
    p5,p6,p7,p8 = a[2],b[2],a[2],b[2]
    p9,p10,p11,p12 = a[3],b[3],a[3],b[3]
    p13,p14,p15,p16 = a[4],b[4],a[4],b[4]
    p17,p18,p19,p20 = a[5],b[5],a[5],b[5]
    ...
    _sdf4(a::Simulation) = (WaterLily.@loop a.flow.σ[I] = cylinder(loc(0,I),p13,p14,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.σ |> Array;)
    _nds_1(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(sim.body,loc(0,I),p1,p2,thk/2) over I ∈ inside(a.flow.p); # use nds1 to obtain the nds view
                            a.flow.f |> Array;)
    _nds_2(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(sim.body,loc(0,I),p5,p6,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_3(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(sim.body,loc(0,I),p9,p10,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_4(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(sim.body,loc(0,I),p13,p14,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_5(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(sim.body,loc(0,I),p17,p18,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
...
run(128,16,8) # run(L,thk,number of segments)