...
function pressure_force3(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for (p1,p2) in segmts
        ...
        WaterLily.@loop sim.flow.f[I,:] .= -sim.flow.p[I].*nds1(loc(0,I,T),p1,p2,R,segmts,n-1,L)/norm(p2-p1) over I ∈ inside(sim.flow.p)
...
function nds1(x,p1,p2,R,segmts,n,L)
    ...
    if angn>=std_ang && ange>std_ang && gg>=R-1 && hh>=R-1 
...
function run(L,h,thk,N)
    ...
    wr = vtkWriter("ThreeD_cylinder_vali_line"; attrib=custom_attrib)
    write!(wr, sim)
    close(wr)

    apply!(x->x[2],sim.flow.p)
    @time global p = pressure_force3(sim,N,thk/2,L)
