...
function pressure_force1(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for i in 1:n-1
        ...
        tann = tan/(norm(tan))
        p2 = sim.body.curve.(s,0) +2.5*tann
        p1 = sim.body.curve.(s,0) -2.5*tann
        push!(a,p1)
        push!(b,p2)
    end
    for i in 1:n-1
        ...
        p1 = a[i]
        p2 = b[i]
        WaterLily.@loop sim.flow.f[I,:] .= 1*norm(nds1(loc(0,I,T),p1,p2,R))/norm(p2-p1) over I ∈ inside(sim.flow.p)
