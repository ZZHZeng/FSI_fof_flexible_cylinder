...
function pressure_force2(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for i in 1:n-1
        s = i*ds
        tan = ForwardDiff.derivative(s->sim.body.curve(s,0),s)
        tann = tan/(norm(tan))
        p1 = sim.body.curve.(s) -8*tann
        p2 = sim.body.curve.(s) +8*tann

        sim.flow.f .= zero(eltype(sim.flow.p))
        apply!(x->x[2],sim.flow.p)
        WaterLily.@loop sim.flow.f[I,:] .= 1*norm(nds1(loc(0,I,T),p1,p2,R)) over I ∈ inside(sim.flow.p)
        push!(forces,sum(T,sim.flow.f,dims=ntuple(j->j,ndims(sim.flow.p)))[:])
        @show(p1)
        @show(p2)
    end
    return forces
...
function make_sim(L,h,thk;Re=250,U =1,mem=Array)
    ...
    knots =   SA_F32[0,0,0,1,1,1] # build by only one curve
    ...
    Simulation((2L,2L,48),(U,0,0),L;U,ν=U*L/Re,body,T=Float64,mem)
end

function run(L,h,thk,N)
    sim = make_sim(L,h,thk) # don't do 48, 9
    ...
    for i in 1:N-1
        ...
        tann = tan/(norm(tan))
        p2 = sim.body.curve.(s,0) +8*tann
        p1 = sim.body.curve.(s,0) -8*tann
        push!(a,p1)
        push!(b,p2)
    end
    p₁,p₂ = a[1],b[1]
    ...
    custom_attrib = Dict(
        "u" => velocity, "p" => pressure, "d" => _body, "nds2" => _nds2, "nds11" => _nds_1, "nds12" => _nds_2, "nds13" => _nds_3
    )
    ...
    @time p = pressure_force2(sim,N,thk/2,L)
