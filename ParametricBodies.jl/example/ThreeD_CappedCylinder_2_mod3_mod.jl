...
function pressure_force1(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for (p₁,p₂) in segmts
        ...
        WaterLily.@loop sim.flow.f[I,:] .= sim.flow.p[I].*nds1(loc(0,I,T),p₁,p₂,R,n,L)/norm(p₂-p₁) over I ∈ inside(sim.flow.p)
...
function run(L,h,thk,N)
    ...
    wr = vtkWriter("ThreeD_cylinder_no_tapper_geometry_method"; attrib=custom_attrib)
    write!(wr, sim)
    close(wr)

 
    @time p = pressure_force1(sim,N,thk/2,L)
