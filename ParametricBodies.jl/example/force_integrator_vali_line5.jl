...
function pressure_force2(sim::Simulation,n,R,L,tapper;T=promote_type(Float64,eltype(sim.flow.p)))
    forces = zeros(n-1,3)
    ds = 1/n
    ...
    for i in 1:n-1
        ...
        if s > tapper
            RR = (R/(tapper-1))*s-R/(tapper-1); 
            cosα = (L*(1-tapper))/√(R^2+L^2*(1-tapper)^2) 
            sinα = (1-(cosα)^2)^0.5
        ...
        else RR = R; cosα = 1; sinα=1; println(-RR^2*π*(beta[1,1])^2); println(-RR^2*π*(beta[1,1]*beta[1,2]))
        end
...
function make_sim(L,thk,h,tapper;Re=250,U=1,mem=Array)
    cps = SA_F32[0  0.5 1 
                 0  h/2 h 
                 0  0.0 0]*L .+ [L,L,16]
    ...
    body = DynamicNurbsBody(curve;thk=(u)->thickness(u,thk,tapper),boundary=false)
    Simulation((4L,2L,32),(U,0,0),L;U,ν=U/Re,body,T=Float64,mem)
end

function run(L,thk,n,h,tapper)
    sim = make_sim(L,thk,h,tapper)
    ...
    @show(π*thk^2/4*1/(1+h^2)^0.5)

    global p = pressure_force2(sim,n,thk/2,L,tapper)
    println("simulation:")
    for i in n-3:n-1
        println(p[i,:])
    end
...
@time run(128,5.037983569881188,24,0.00,0.9282272561276117)