...
function pressure_force1(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for i in 1:n-1
        ...
        tann = tan/(norm(tan))
        p1 = sim.body.curve.(s,0) -4*tann
        p2 = p1 + 8*tann
        p3 = p1 + 4*tann - 1*tann # cylinder around gauss point
        p4 = p3 + 2*tann
        push!(a,p1)
        push!(b,p2)
        push!(c,p3)
...
function run(L,h,thk,N)
    ...
    _sdf(a::Simulation) = (WaterLily.@loop a.flow.σ[I] = cylinder(loc(0,I),p1,p2,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.σ |> Array;)
    _nds_1(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(loc(0,I),p1,p2,p3,p4,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_2(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(loc(0,I),p5,p6,p7,p8,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_3(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(loc(0,I),p9,p10,p11,p12,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_4(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(loc(0,I),p13,p14,p15,p16,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds_5(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= nds1(loc(0,I),p17,p18,p19,p20,thk/2) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)
    _nds2(a::Simulation) = (WaterLily.@loop a.flow.f[I,:] .= WaterLily.nds(a.body,loc(0,I),0.0) over I ∈ inside(a.flow.p); 
                            a.flow.f |> Array;)

    custom_attrib = Dict(
        "u" => velocity, "p" => pressure, "d" => _body, "nds2" => _nds2, "nds11" => _nds_1, "nds12" => _nds_2, "nds13" => _nds_3, "nds14" => _nds_4, "nds15" => _nds_5, "cy1" => _sdf
    )
