...
function pressure_force1(sim::Simulation,n,R,L;T=promote_type(Float64,eltype(sim.flow.p)))
    ...
    for i in 1:n-1
        ...
        tann = tan/(norm(tan))
        p2 = sim.body.curve.(s,0) +2*tann
        p1 = sim.body.curve.(s,0) -2*tann
        push!(a,p1)
        push!(b,p2)
    end
    for i in 1:n-1
        ...
        # push!(sufa,sum(T,sim.flow.f,dims=ntuple(j->j,ndims(sim.flow.p)))[:])
    end
    return forces
end
...
function nds1(x,p1,p2,R)
    ...
    if abs(norm(aa)*angn-norm(cc)*ange)<=2 && angn>0 && ange>0
...
function make_sim(L,h,thk;Re=250,U =1,mem=Array)
    ...
    Simulation((4L,4L,64),(U,0,0),L;U,ν=U*L/Re,body,T=Float64,mem)
end
