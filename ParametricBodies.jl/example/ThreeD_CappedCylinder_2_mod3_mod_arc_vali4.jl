...
function make_sim(L,h,thk;Re=250,U =1,mem=Array)
    ...
    Simulation((4L,2L,128),(U,0,0),L;U,ν=U*L/Re,body,T=Float64,mem)
end
...
function run(L,h,thk,N)
    ...
    @show((sin(π/8))^2*π*(thk/2)^2)
    @show((sin(2π/8))^2*π*(thk/2)^2)
    @show((sin(3π/8))^2*π*(thk/2)^2)
    @show(1.0*π*(thk/2)^2)
    @show(p./8)
end
